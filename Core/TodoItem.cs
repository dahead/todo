...
public class TodoItem
{
    ...
    public string DefaultDateFormatString = "MMM dd yyyy hh:mm:ss";
    ...
    public string GetDueText()
    {
        ...
        {
            string result = this.DueAt.ToString(this.DefaultDateFormatString);
    ...
    public Tree GetDetailsTree(bool repetations = false, bool attachments = false, bool notes = false, bool calendar = false)
    {
        ...
        if (calendar)
        {
            var duenode = tree.AddNode($"[yellow]Due at:[/] { this.DueAt.ToLongDateString() }");
            duenode.AddNode(new Calendar(this.DueAt.Year, this.DueAt.Month)
                .Border(TableBorder.Rounded)
